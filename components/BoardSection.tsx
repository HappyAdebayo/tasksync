...
function BoardCard({
  ...
  return (
    <div className="group relative flex flex-col overflow-hidden rounded-xl border border-[#EAECF0] bg-white text-left transition-all hover:-translate-y-0.5 hover:border-[#D0D5DD] hover:shadow-[0_8px_20px_rgba(23,26,33,0.06)]">
      <button onClick={() => onSelect(board.id)} className="flex flex-1 flex-col text-left">
        ...
        <div className="flex flex-1 flex-col gap-5 p-5">
          <div>
            <h3 className="font-[family-name:var(--font-display)] text-[16px] font-semibold text-[#101828]">
              {board.name}
            </h3>
...
export default function BoardsSection({
  ...
  return (
    <section className="mx-auto max-w-[1100px] px-7 py-8">
      ...
      <button
        onClick={onBack}
        className="mb-4 flex items-center gap-1 text-[13px] font-medium text-[#667085] transition-colors hover:text-[#4C5FD5]"
      >
      ...
      <div className="mb-5 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <h2 className="font-[family-name:var(--font-display)] text-[20px] font-semibold text-[#101828]">
      ...
      {boards.length === 0 ? (
        <div className="flex flex-col items-center gap-2 rounded-2xl border border-dashed border-[#D0D5DD] px-6 py-14 text-center">
          <LayoutGrid className="h-6 w-6 text-[#98A2B3]" />
          <p className="text-[13.5px] font-medium text-[#101828]">No boards yet</p>
          <p className="text-[12.5px] text-[#667085]">Create the first board for this workspace.</p>
        </div>
      ) : (
